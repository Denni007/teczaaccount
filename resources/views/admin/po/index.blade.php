...
@section('content')
    ...
    <div class="card m-2">
        ...
        @if(empty($permissions))
             <a href="{{ route('admin.purchase-order.add') }}" class="btn btn-primary float-right"> Add Purchase Order</a>
...
<script>
    const delete_url = "{{ route('admin.purchase-order.delete') }}";
    const approve_url = "{{ route('admin.purchase-order.approve') }}";
    const reject_url = "{{ route('admin.purchase-order.reject') }}";
    ...
    $(document).ready( function () {
        companyTable = $('#datatable').DataTable({
            ...
            bSort:false,
            ajax: {
                url: "{{ route('admin.purchase-order.filter') }}",
                type: 'POST',
                headers: {
        ...
        $(document).on('click','.po-approve', function() {
        var id= $(this).data('id');
        var companyTable_row = $(this).closest("tr");
        Swal.fire({
            title: 'Are you sure?',
            text: "You want to approve this purchase order ?",
             type: 'warning',
            //icon: 'warning',
            showCancelButton: true,
        ...
        $(document).on('click','.po-reject', function() {
        var id= $(this).data('id');
        var companyTable_row = $(this).closest("tr");
        Swal.fire({
            title: 'Are you sure?',
            text: "You want to reject this purchase order ?",
             type: 'warning',
            //icon: 'warning',
            showCancelButton: true,
