...
@section('content')
    ...
    <div class="card m-2">
        ...
        @if(empty($permissions))
            <a href="{{ route('admin.invoice.add') }}" class="btn btn-primary float-right"> Add Invoice</a>
...
<script>
    const delete_url = "{{ route('admin.invoice.delete') }}";
    const approve_url = "{{ route('admin.invoice.approve') }}";
    const reject_url = "{{ route('admin.invoice.reject') }}";
    ...
    $(document).on('click','.invoice-reject', function() {
        var id= $(this).data('id');
        var companyTable_row = $(this).closest("tr");
        Swal.fire({
            title: 'Are you sure?',
