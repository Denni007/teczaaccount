   ...
   <section class="content-header">
      <div class="container-fluid">
         <div class="row mb-2">
            <div class="col-sm-6">
               <h1>
                  Purchase {{ isset($dataArr) ? "Edit" : "Add" }}
               </h1>
            </div>
            <div class="col-sm-6">
               <ol class="breadcrumb float-sm-right">
                  <li class="breadcrumb-item"><a href="{{ route('admin.index') }}">Home</a></li>
                  <li class="breadcrumb-item"><a href="{{ route('admin.purchase') }}">Purchase</a>
                  <li class="breadcrumb-item active">Purchase {{ isset($dataArr) ? "Edit" : "Add" }}</li>
               </ol>
            </div>
         </div>
      </div><!-- /.container-fluid -->
      ...
      <div class="row d-flex justify-content-center">
         <div class="col-md-10">
            <div class="card card-primary">
               ...
               <div class="card-body">
                  <form class="add-purchase-form" action="{{ route('admin.purchase.store') }}" method="POST" enctype='multipart/form-data'>
                     @csrf
                     @method("POST")
                     ...
                     <div class="row">
                        ...
                        <div class="col-6 form-group">
                           ...
                           <select class="form-control select2" name="po" id="po" onchange="fetch_purchase_details();">
                              <option value="">Select Purchase order</option>
                              @foreach($poInvoiceDetails as $po)
                              <option value="{{$po->id}}" {{ (@$dataArr->po_id == $po->id) ? 'selected':'' }}>{{$po->bill_no}}</option>
                              @endforeach
                           </select>
                        </div>
                     </div>
                           ...
                           @if(Auth::user()->type == 2)
                              ...
                              @else
                                 <select class="form-control select2" name="bill_type" id="bill_type" onchange="displayBankList()">
                                    <option value="">Select Bill Type</option>
                                    <option value="1" {{ (@$dataArr->bill_type == 1) ? 'selected':'' }}>Cash</option>
                                    <option value="2" {{ (@$dataArr->bill_type == 2) ? 'selected':'' }}>Bank</option>
                                 </select>
                              @endif
                           @else
                           <select class="form-control select2" name="bill_type" id="bill_type" onchange="displayBankList()">
                     ...
                     <div class="row">
                        <div class="col-12 form-group">
                           <label for="total_amount">Purchase Invoice</label>
...
<script type="text/javascript">
   ...
   // });
   function fetch_purchase_details()
   {
      var po = $('#po').val();
      if(po != '')
      {
         var csrfToken = '{{ csrf_token() }}';
         var url = "{{route('admin.purchase.fetch-purchase-details')}}";
         jQuery.ajax({
           url:  url,
           type: "POST",
           data: {
           ...
           success: function(response)
           {
               if(response.status == 1)
               {
                  ...
                  $('#rawmaterial').html(response.materialhtml);
                  $('#otherproduct').html(response.prodcuthtml);
                  calculateAmount();
                  $(".js-example-tags").select2({
                     tags: true
                  });
