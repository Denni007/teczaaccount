        ...
        <div class="card-header header-elements-inline">
            <div class="d-flex justify-content-between align-items-center">
                ...
                @if(empty($permissions))
                    <a href="{{ route('admin.receipt.add') }}" class="btn btn-primary float-right"> Make Receipt</a>
                @elseif(!empty($permissions) && in_array('receipt_add',$permissions))
                    <a href="{{ route('admin.receipt.add') }}" class="btn btn-primary float-right"> Make Receipt</a>
                @endif
            </div>
        </div>
...
<script>
    const delete_url = "{{ route('admin.receipt.delete') }}";
    const approve_url = "{{ route('admin.receipt.approve') }}";
    const reject_url = "{{ route('admin.receipt.reject') }}";
    var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
    var companyTable = "";
    $(document).ready( function () {
        loadData();

        $(document).on('click','.receipt-delete', function() {
            var id= $(this).data('id');
        ...
        $(document).on('click','.receipt-approve', function() {
            var id= $(this).data('id');
            var companyTable_row = $(this).closest("tr");
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to approve this receipt ?",
                type: 'warning',
                //icon: 'warning',
                showCancelButton: true,
        ...
        $(document).on('click','.receipt-reject', function() {
            var id= $(this).data('id');
            var companyTable_row = $(this).closest("tr");
            Swal.fire({
                title: 'Are you sure?',
                text: "You want to reject this receipt?",
                type: 'warning',
                //icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'Yes, delete it!',
    ...
    function loadData() {
        ...
        companyTable = $('#datatable').DataTable({
            ...
            bSort:false,
            ajax: {
                url: "{{ route('admin.receipt.filter') }}",
                type: 'POST',
                headers: {
            ...
            columns: [
                ...
                { data: 'receipt_type', name: 'receipt_type', className: 'text-center', searchable:false, orderable:false },
                ...
                { data: 'receipt_date', name: 'receipt_date', className: 'text-center', searchable:false, orderable:false },
